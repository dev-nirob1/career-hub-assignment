    ...
    return (
        <div className='flex justify-between items-center container mx-auto h-20 px-5 bg-gray-50'>
            ...
            <nav className={`md:flex md:items-center font-medium text-base lg:text-lg w-full md:w-auto absolute md:relative top-20 md:top-0 right-0 transition-all duration-300 ease-in-out ${open ? 'bg-gray-200' : ''} ${open ? 'block' : 'hidden'}`}>
                ...
                <NavLink to="/applied-job" className={`px-3 py-2 rounded block md:inline-block mt-4 md:mt-0 mx-1 lg:mx-2 text-gray-700 hover:text-blue-500 ${!open ? 'hover:bg-gray-100' : ''}`}>Applied Jobs</NavLink>

                <NavLink to="/blog" className={`px-3 py-2 rounded block md:inline-block mt-4 md:mt-0 mx-1 lg:mx-2 text-gray-700 hover:text-blue-500 ${!open ? 'hover:bg-gray-100' : ''}`}>Blog</NavLink>
            </nav>

            <button className='text-base lg:text-[20px] text-white px-3 md:px-5 py-[10px] md:py-4 bg-gradient-to-r from-[#7E90FE] to-[#9873FF] font-semibold rounded-lg transition-all duration-300 ease-in-out'>Start Applying</button>
        </div>
    );
};
